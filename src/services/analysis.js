...
export class AnalysisService {
  ...
  async analyzeCodeQuality(prDetails, filePaths = null) {
    const filesToAnalyze = filePaths || prDetails.files.map(f => f.filename);
    const analysis = {
      overall_score: 0,
      files: [],
      summary: {
  ...
  async analyzeFileQuality(file) {
    ...
    analysis.metrics.cyclomatic_complexity = this.calculateComplexity(addedLines, analysis.language);
    analysis.metrics.maintainability_index = this.calculateMaintainabilityIndex(addedLines, analysis.language);
    analysis.metrics.technical_debt_ratio = this.calculateTechnicalDebt(addedLines, analysis.language);

    // Detect issues
    analysis.issues = this.detectCodeIssues(addedLines, analysis.language);
    analysis.suggestions = this.generateCodeSuggestions(addedLines, analysis.language);
  ...
  async analyzeTestCoverage(prDetails) {
    ...
    analysis.coverage_estimate = this.estimateTestCoverage(analysis.test_files, analysis.production_files);
    analysis.missing_tests = this.identifyMissingTests(analysis.production_files, analysis.test_files);
  ...
  async generateSuggestions(prDetails, filePath, focusAreas = []) {
    ...
    const areas = focusAreas.length > 0 ? focusAreas : ['performance', 'security', 'maintainability', 'readability', 'testing'];

    for (const area of areas) {
      suggestions.categories[area] = this.generateAreaSpecificSuggestions(file, area, language);
    }

    // Prioritize suggestions
    suggestions.priority_suggestions = this.prioritizeSuggestions(suggestions.categories);
    suggestions.implementation_examples = this.generateImplementationExamples(suggestions.priority_suggestions, language);
  ...
  detectPrimaryLanguage(files) {
    ...
    return Object.entries(languageCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || 'unknown';
  }
  ...
  calculateMaintainabilityIndex(lines, language) {
    ...
    if (codeText.match(/\w{30,}/)) { // Very long variable names
      maintainability -= 3;
    }
  ...
  calculateTechnicalDebt(lines, language) {
    let debtScore = 0;
    const codeText = lines.join('\n');
  ...
  calculateOverallScore(fileAnalyses) {
    ...
    const avgMaintainability = fileAnalyses.reduce((sum, f) => sum + f.metrics.maintainability_index, 0) / fileAnalyses.length;
    const avgComplexity = fileAnalyses.reduce((sum, f) => sum + f.metrics.cyclomatic_complexity, 0) / fileAnalyses.length;
    const avgDebt = fileAnalyses.reduce((sum, f) => sum + f.metrics.technical_debt_ratio, 0) / fileAnalyses.length;
  ...
  detectCodeIssues(lines, language) {
    const issues = [];
    const codeText = lines.join('\n');
  ...
  calculateOverallRisk(analysis) {
    ...
    if (analysis.impact_categories.security_sensitive.length > 0) riskScore += 30;
  ...
  analyzeFilePatterns(file, language) {
    const patterns = {
      good_patterns: [],
      anti_patterns: [],
      architectural_issues: [],
  ...
  mergePatternAnalysis(analysis, filePatterns) {
    ...
    analysis.patterns_found.architectural_issues.push(...filePatterns.architectural_issues);
    analysis.patterns_found.design_patterns.push(...filePatterns.design_patterns);
  }
  ...
  isDependencyFile(filename) {
    return this.dependencyPatterns.package_files.includes(filename) ||
  ...
  mergeDependencyAnalysis(analysis, depAnalysis) {
    ...
    analysis.dependency_changes.security_issues.push(...depAnalysis.security_issues);
  }
  ...
  generateTestRecommendations(analysis) {
    ...
    if (analysis.coverage_estimate < 70) {
      recommendations.push("Consider adding more test coverage for the changed code");
    }

    if (analysis.missing_tests.length > 0) {
      recommendations.push(`Add tests for: ${analysis.missing_tests.map(t => t.filename).join(', ')}`);
    }
  ...
  generateAreaSpecificSuggestions(file, area, language) {
    const suggestions = [];
    ...
    switch (area) {
      case 'performance':
        if (codeText.includes('for (') && codeText.includes('length')) {
          suggestions.push({
            type: 'performance',
            description: 'Cache array length in loop conditions for better performance',
      ...
      case 'maintainability':
        if (codeText.match(/function\s+\w+\s*\([^)]{50,}\)/)) {
          suggestions.push({
            ...
            description: 'Consider breaking down functions with many parameters',
            example: 'Use configuration objects instead of long parameter lists'
          });
        }
        break;
